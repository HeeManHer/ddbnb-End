...
public class TokenProvider {
    ...
    public AccessTokenDTO generateMemberTokenDTO(MemberDTO foundmember) {
        ...
        Claims claims = Jwts.claims().setSubject(String.valueOf(foundmember.getMemberId()));
        long now = (new Date()).getTime();

        // Access Token 생성
        Date accessTokenExpiresIn = new Date(now + ACCESS_TOKEN_EXPIRE_TIME);
        String jwtToken = Jwts.builder().setClaims(claims).setExpiration(accessTokenExpiresIn).signWith(key, SignatureAlgorithm.HS512).compact();

        return new AccessTokenDTO(BEARER_TYPE, foundmember.getMemberId(), jwtToken, accessTokenExpiresIn.getTime());
    }
